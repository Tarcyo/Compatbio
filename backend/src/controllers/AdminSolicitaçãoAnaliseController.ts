...
function toNullablePriority(v: any): number | null {
  ...
  // ajuste a regra se quiser (1..5 é comum)
 ...
 * - só permite responder se STATUS atual for PENDENTE
 * - status de resposta NÃO pode ser PENDENTE
 */
 ...
 * Body: { id, status, descricao?, prioridade? }
 *
 * ✅ NOVO: cria OU atualiza o resultado_catalogado (químico+biológico) após responder.
 */
export async function responderSolicitacaoAnalise(req: Request, res: Response) {
  ...
  // detectar se a chave veio no body (pra permitir “não mexer” quando não enviado)
  ...
  // descricao: string → trim, vazio vira null, null limpa
  ...
  // prioridade: number 1..5 ou null (limpar)
  ...
  if (hasPrioridade) {
    ...
    if (raw === null) {
      ...
      if (parsed === null) {
        return res
          .status(400)
          .json({ error: "prioridade inválida. Use inteiro de 1 a 5, ou null." });
      }
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      // 1) garante que existe e pega os IDs dos produtos (para o catálogo)
      ...
      // 2) atualiza a solicitação (permite corrigir quantas vezes quiser)
      ...
      // 3) ✅ cria OU atualiza o catálogo (resultado_catalogado) para a combinação
      ...
      if (idQuimico && idBiologico) {
        // Atualiza se existir (status sempre; descrição só se veio no body)
